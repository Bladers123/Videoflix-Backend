...
def convert_to_120p(source):
    ...
    cmd_mp4 = 'ffmpeg -i "{}" -s 160x120 -c:v libx264 -crf 23 -c:a aac -strict -2 "{}"'.format(
        source, mp4_target)
    subprocess.run(cmd_mp4, capture_output=True)
    print('120p: in mp4 umgewandelt')

    cmd_hls = 'ffmpeg -i "{}" -c:v copy -c:a aac -ac 2 -start_number 0 -hls_time 10 -hls_list_size 0 -f hls "{}"'.format(
        mp4_target, hls_target)
...
def convert_to_360p(source):
    ...
    cmd_mp4 = 'ffmpeg -i "{}" -s 640x360 -c:v libx264 -crf 23 -c:a aac -strict -2 "{}"'.format(
        source, mp4_target)
    subprocess.run(cmd_mp4, capture_output=True)
...
def convert_to_720p(source):
    ...
    cmd_mp4 = 'ffmpeg -i "{}" -s 1280x720 -c:v libx264 -crf 23 -c:a aac -strict -2 "{}"'.format(
        source, mp4_target)
    subprocess.run(cmd_mp4, capture_output=True)
...
def convert_to_1080p(source):
    ...
    cmd_mp4 = 'ffmpeg -i "{}" -s 1920x1080 -c:v libx264 -crf 23 -c:a aac -strict -2 "{}"'.format(
        source, mp4_target)
    subprocess.run(cmd_mp4, capture_output=True)
...
def upload_hls_files(directory, video_folder, video_type):
    ...
    for filename in os.listdir(directory):
        if (filename.endswith(".m3u8") or filename.endswith(".ts")) and filename.startswith(video_folder):
            ...
            print(
                f"{filename} hochgeladen in /movies/{video_folder}/{remote_filename}")
...
def upload_master_playlist(directory, video_folder, base_name, video_type):
...
def upload_thumbnail(source, video_folder, video_type, remote_filename=None):
    ...
    try:
        ...
        print(
            f"Thumbnail '{source}' erfolgreich hochgeladen als {base_remote_dir}/{video_folder}/{remote_filename}")
...
def delete_local_media_folder():
    media_root = settings.MEDIA_ROOT
    if os.path.isdir(media_root):
        try:
            shutil.rmtree(media_root)
            print(f"MEDIA_ROOT '{media_root}' erfolgreich gelöscht.")
        except Exception as e:
            print(f"Fehler beim Löschen von MEDIA_ROOT '{media_root}': {e}")
    else:
        print(f"MEDIA_ROOT '{media_root}' existiert nicht oder ist kein Verzeichnis.")
